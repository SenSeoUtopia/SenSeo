...
return Response::json("Unable to get Data");
}
...
// Ajax Link Preview
public function link_preview($f3){

SetUp::init();

$text = $f3->get("POST.text");
$imageQuantity = $f3->get("imagequantity");
$text = " " . str_replace("\n", " ", $text);
$header = "";
...
// Ajax Search
public function ajax_search($f3){

if(!$f3->exists('GET.q')){
$keyword = $f3->get('POST.q');
} else {
...
$table = $f3->get("db_prefix")."users";

$query['user'] = User::where("user_name", "like" ,"%$keyword%")->get(array("user_name","display_name","avatar"));

return Response::json($query);

} else {
return Response::json(array("No Results Found!"));
}
...
public function ajax_posting($f3){
...
$check = Post::where(array("user_id" => $user_id, "content" => $msg))->count();
...
if($post->save()){
$message = array("success" => true,"msg" => "Your Message Posted");
} else {
$message = array("success" => false,"msg" => "Something is wrong. Please try again later.");
}

} else {
$message = array("success" => false,"msg" => "Post posted already.");
}
	
} else {
$message = array("success" => false,"msg" => "Type your Message.");
}
...
/* Ajax Posting Photo */
public function ajax_posting_photo($f3){

$data = $f3->get('POST');

$files = $f3->get('FILES.photos');
...
$check = Post::where(array("user_id" => $user_id, "content" => $msg))->exists();
...
/* Ajax Posting Questions */
public function ajax_question($f3){
...
$get_answer = $data['answers'];

$message = $get_answer;

$user_id = "1";

$question = Question::firstOrCreate(["user_id" => $user_id,"body" => $question_title]);

$question_id = $question->id;

foreach($get_answer as $answers){

$answer = Answer::firstOrNew(["body" => $answers,"question_id" => $question_id]);
$answer->votes = 0;

if($answer->save()){ $message = array("success" => true,"msg" => "Questions and Answers created successfully"); } else { $message = array("success" => false,"msg" => "unable to create answers"); }
...
/* Ajax Commenting */
public function ajax_comment($f3){
$home_url = $this->home_url;
...
'post_id' => 'required|numeric|integer',
'post_type' => 'required|numeric|integer',
'msg' => 'required'
));
...
case 1: $set_post_type = 'post'; break;
case 2: $set_post_type = 'photo'; break;
case 3: $set_post_type = 'album'; break;
case 4: $set_post_type = 'video'; break;
case 5: $set_post_type = 'poll'; break;
default: $set_post_type = 'post'; break;
}
...
$user_id = $get_user->id;
$commenter_user = $get_user->display_name;

$user = User::find($poster_id);
$user_name = $user->user_name;

$post_url = "$home_url/$user_name/posts/$post_id";
$content = "$commenter_user has commented on your $set_post_type";

$msg = $data['msg'];

$comment = Comment::firstOrNew(array("user_id" => $user_id,"post_id" => $post_id));
$comment->content = $msg;
$comment->save();

$notification = Notification::firstOrNew(array("user_id" => $poster_id,"post_id" => $post_id));
$notification->post_url = $post_url;
$notification->post_type = $get_post_type;
$notification->save();
...
/* Ajax Get Latest Post */
public function ajax_like($f3){
...
$user_full_name = $get_user->display_name;
$post_id = $data['post_id'];	
$post_type = $data['post_type'];

if($action === "like"){
switch($post_type){
case 'post': $set_post_type = 1; break;
case 'photo': $set_post_type = 2; break;
case 'album': $set_post_type = 3; break;
case 'video': $set_post_type = 4; break;
case 'poll': $set_post_type = 5; break;
default: $set_post_type = 1; break;
}

$like = Like::firstOrNew(array('post_id' => $post_id,'user_id' => $user_id));
$like->post_type = $set_post_type;
$like->liked = 1;
$like->save();

$content = "$user_full_name has liked your $post_type";
$post_url = "$home_url/$user_name/posts/$post_id";

$notification = new Notification();
$notification->post_type = $set_post_type;
...
$btn = "<a href=\"javascript:void(0);\" id=\"unlike\" rel=\"$post_id\" post_type=\"$post_type\"><span class=\"glyphicon glyphicon-hand-right\"></span> Unike</a>";

return Response::json(array($btn));
} else if($action === "dislike"){

$like = Like::where(array("user_id" => $user_id,"post_id" => $post_id))->first();
$like->delete();

$btn = "<a href=\"javascript:void(0);\" id=\"like\" rel=\"$post_id\" post_type=\"$post_type\"><span class=\"glyphicon glyphicon-hand-right\"></span> Like</a>";
return Response::json(array($btn));
} else {
return Response::json(array("error" => true,"msg" => "Invalid Action"));
}
...
/* Album Images Upload */
public function album_photos($f3){
...
$album_id = $f3->get("POST.album_id");

if(count($file_list['files']['name']) > 1){

foreach($file_list as $key => $file){

$file_name = pathinfo($file_list['files']['name'][$key], PATHINFO_FILENAME);
$file_name = preg_replace("/\W+/isu",'-', $file_name);
$temp_name = $file['files']['tmp_name'][$key];
$ext = pathinfo($file_list['files']['name'][$key], PATHINFO_EXTENSION);
$file_name = "$file_name.$ext";

$destination_path = "$upload_dir/$user_id/photos/$file_name";

$file_path = "$home_url/uploads/$user_id/photos/$file_name";

if(move_uploaded_file($temp_name,$destination_path)){

$image = Photo::firstOrNew(["file_name" => $file_name,"user_id" => $user_id]);
...
$msg = "Unable to Upload Multiple files";
...
$file_name = pathinfo($file_list['files']['name'], PATHINFO_FILENAME);
$file_name = preg_replace("/\W+/isu",'-', $file_name);
$temp_name = $file_list['files']['tmp_name'];
$ext = pathinfo($file_list['files']['name'], PATHINFO_EXTENSION);
$file_name = "$file_name.$ext";
...
$msg = array("msg" => "Image has been Uploaded Successfully");	
...
// Ajax Notification
public function notification($f3){

$user_id = $f3->exists("GET.user_id") ? $f3->get("GET.user_id") : 0;
...
// Ajax Friend Request
public function friend_request($f3){

$user_id = $f3->exists("user_id") ? $f3->get("user_id") : 0;

$data = Friend::where("user_id = ?",array($user_id));
...
// Ajax Get All Notifications
public function get_all_notification($f3){
...
if($user){
$notification = $user->notifications()->unread()->get(array("content","post_url","created_at"));
$friend_request = $user->friend_request()->unread()->get();
$messages = $user->messages()->unread()->get();
} else {
...
// Ajax People Suggestion
public function people($f3){
...
$query = User::where("user_name", "like" ,"%$keyword%")->get(array("user_name","display_name"));
...
// Ajax Template
public function ajax_template($f3){
